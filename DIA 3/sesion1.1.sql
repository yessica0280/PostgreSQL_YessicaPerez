...
INSERT INTO ejemplo VALUES(1, 'adrian', 'descripcion', 10.23, true, '2025-02-23', '13:10:50', '2025-02-23 13:10:50', '2025-02-23 13:10:50+05', '1 day', 
'102.168.1.1', '80:00:12:23:10:00', '(10, 20)', '{"key": "value"}', 'f66c844a-fd5b-48c3-8c46-735e41b0a2e6', '15.23', '[10, 20)', ARRAY['rojo', 'verde', 'amarrillo'], 'Masculino');
...
alter table credenciales add constraint fk_username foreign key(username) references credenciales(username);
...
select string_agg(substring('0123456789abcdefghijklmnñopqrstuvwxyz$.!', round(random() * length('0123456789abcdefghijklmnñopqrstuvwxyz$.!'))::integer, 1), '') as newPassword from generate_series(0, 5);
...
BEGIN
	return query select string_agg(substring('0123456789abcdefghijklmnñopqrstuvwxyz$.!', round(random() * length('0123456789abcdefghijklmnñopqrstuvwxyz$.!'))::integer, 1), '') 
	as newPassword from generate_series(0, long); --les queda para revisar.
END;
$$ LANGUAGE plpgsql;
...
returns TEXT as $$
BEGIN
	return (select string_agg(substring('0123456789abcdefghijklmnñopqrstuvwxyz$.!', round(random() * length('0123456789abcdefghijklmnñopqrstuvwxyz$.!'))::integer, 1), '') 
	as newPassword from generate_series(0, long)); --les queda para revisar.
END;
$$ LANGUAGE plpgsql;

-- retornar texto pero de una variable?
create function generate_random_password2(long INTEGER)
returns TEXT as $$
declare res text;
BEGIN
	res = (select string_agg(substring('0123456789abcdefghijklmnñopqrstuvwxyz$.!', round(random() * length('0123456789abcdefghijklmnñopqrstuvwxyz$.!'))::integer, 1), '') 
	as newPassword from generate_series(0, long)); --les queda para revisar.
	return res;
END;
$$ LANGUAGE plpgsql;
...
update credenciales set password = generate_random_password1(12); -- where username = 'brush99'; 

select sha512(password::bytea), username from credenciales;

update credenciales set password = sha512(generate_random_password1(12)) where username = 'brush99'; 
...
select * from credenciales where sha512('lcla2abv52zlf'::bytea) = password_sha and username = 'brush99';
